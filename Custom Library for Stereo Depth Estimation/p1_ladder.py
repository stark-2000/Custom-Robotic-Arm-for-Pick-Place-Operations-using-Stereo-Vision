...
class stereo_depth_estimation:
    def __init__(self):
        ...
        self.baseline = 228.38 #baseline length in mm
        self.focal_length = 1734.16 #focal length in px

        image_path=['../Dataset/ladder/im0.png','../Dataset/ladder/im1.png'] #Path of the images
        self.imgs = []
    ...
    def Correspondence_SSD(self, left_img, right_img, kernel, max_offset):
        ...
        Image.fromarray(disparity).save('output/ladder/disparity_map_gray.png')
...
def main():
    ...
    cv2.imwrite('output/ladder/rectified_left_image.png', rec_left)
    cv2.imwrite('output/ladder/rectified_right_image.png', rec_right)
    ...
    p1.Correspondence_SSD("output/ladder/rectified_left_image.png", "output/ladder/rectified_right_image.png", 6, 30)
    
    #Heat Map Conversion - Disparity Map:
    disparity_img_gray = cv2.imread("output/ladder/disparity_map_gray.png")
    disparity_img_heatmap = cv2.applyColorMap(disparity_img_gray, cv2.COLORMAP_JET)

    #Saving the disparity map - color:
    cv2.imwrite('output/ladder/disparity_map_color.png', disparity_img_heatmap)
    ...
    cv2.imwrite('output/ladder/depth_map_gray.png', depth_map_gray)
    ...
    cv2.imwrite('output/ladder/depth_map_color.png', depth_map_heatmap)
